...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
      ...
      const fetchError= res.errors;
      if(fetchError){
        setError(fetchError[Object.keys(fetchError)[0]][0])
        return;
      }
  ...
  return (
    <div className="h-[100vh] w-full bg-[#eee] flex justify-center items-center">
      <form onSubmit={handleSubmit} className="p-14 flex flex-col justify-around bg-white w-[625px] h-[640px] shadow rounded-xl">
        ...
        <input className="p-2 text-center outline outline-[#aaa] outline-solid rounded-md" name="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Juan Coronado" autoComplete="name" />
        ...
        <button className="font-sans p-2 bg-amber-500 text-white rounded-md" type="submit">Sign up!</button>
        {<p className="font-sans h-[24px] text-red-500">{error}</p>}
        <p className="text-[#777]">Already have an account? <a className="text-amber-500" href="/login">Sign in</a></p>
      </form>
    </div>
